    ...
    useEffect(() => {
        ...
        const loadConsultation = async () => {
            ...
            try {
                ...
                if (!citaId) {
                    throw new Error(
                        "No se ha indicado una consulta válida."
                    );
                }

                if (!storedUser || !token) {
                    throw new Error(
                        "Debes iniciar sesión para acceder a la teleconsulta."
                    );
                }
                ...
                if (!response.ok) {
                    throw new Error(
                        data.error || "No se pudo validar la consulta."
                    );
                }
        ...
        return (
            <div className="text-white py-5 min-vh-100 d-flex align-items-center justify-content-center p-4">
                <div className="alert alert-danger mb-0" role="alert">
                    ...
                    <div>
                        {error || "No se pudo validar la consulta."}
                    </div>
                </div>
            </div>
        );
            ...
            <div className="container-fluid">
                ...
                <div className="bg-white bg-opacity-10 border border-secondary border-opacity-50 rounded-4 shadow-sm overflow-hidden">
                    ...
                    <div className="p-2 bg-dark bg-opacity-50">
                        <div
                            className="rounded-3 overflow-hidden border border-secondary border-opacity-50"
                            style={{ height: "650px" }}
                        >
                            <JitsiCall
                                roomName={roomName}
                                ...
                                onConferenceLeft={() => {
                                    console.log(
                                        "Teleconsulta: Jitsi desconectado"
                                    );
                                ...
                                onConferenceError={() => {
                                    console.error(
                                        "Teleconsulta: error de conexión"
                                    );
